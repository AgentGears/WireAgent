...
when the target is absent *and* an explicit deletion tombstone is visible
outside any article. Everything else that is not one unique target article is
``unknown``.
"""
    ...
    async def read_delete_state(self, post_url: str, post_id: str) -> ActionResult:
        ...
        async def operation() -> ActionResult:
            ...
            expr = (
                ...
                "var t=(n.innerText||'').trim();if(phrases[t])"
                "return JSON.stringify({status:'deleted',tombstone:t});}"
            ...
            for _ in range(20):
                ...
                last = parsed
                if parsed.get("status") in {"present", "deleted", "ambiguous"}:
                    break
            ...
            if status == "deleted":
                return ok_result(
                    data={
                        "post_state": "deleted",
                        "target_post_id": post_id,
                        "evidence": "explicit_delete_tombstone",
                        "tombstone": last.get("tombstone"),
                    }
                )
            return soft_failure(
                "delete state could not be established from explicit target evidence",
                failure_category=FailureCategory.UNKNOWN,
            )
