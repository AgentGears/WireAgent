"""Integration tests for the Layer-5 target-bound quote executor."""
...
class _QuoteBroker:
    ...
    async def open_quote_on_target(
        ...
        if self.mode == "open_failure":
            return soft_failure("quote unavailable", failure_category=FailureCategory.SECURITY)
    ...
    async def attach_media(self, image_path: str) -> ActionResult:
        raise AssertionError(f"plain quote must not attach media: {image_path}")
...
class _Evidence:
    ...
    async def capture_new_post(
        ...
        *,
        ...
        return ok_result(
            data={
                "post_id": self.quote_post_id,
                "post_url": f"https://x.com/actor/status/{self.quote_post_id}",
            }
        )
    ...
    async def verify_quote_attachment(
        ...
        *,
        ...
        self.verify_calls += 1
        assert quote_post_id == self.quote_post_id
        assert target_post_id == "123"
        assert actor_id == "@actor"
        assert normalized_text == self.expected_text
        if self.raise_verify:
            raise RuntimeError("verification interrupted")
        if not self.verify_ok:
            return soft_failure(
                "quote attachment proof failed",
                failure_category=FailureCategory.UNKNOWN,
            )
        data = {
            "quote_attachment_verified": self.attachment_verified,
            "target_post_id": "123",
            "quote_post_id": self.quote_post_id,
            "text_matches": True,
        }
        if self.include_actor:
            data["quote_actor"] = "actor"
        if self.include_url:
            data["quote_url"] = f"https://x.com/actor/status/{self.quote_post_id}"
        return ok_result(data=data)


def _intent(
    text: str = "hello quote",
    *,
    post_url: str = "https://x.com/target/status/123",
) -> WriteIntent:
    risk, compensation = DEFAULT_REGISTRY.require("quote")
    return WriteIntent(
        action_type="quote",
        target_type="post",
        target_id="123",
...
def _executor(
    ...
    *,
    broker: _QuoteBroker,
    evidence: _Evidence,
) -> tuple[M5QuoteExecutor, EffectLedger]:
    ...
    return M5QuoteExecutor(runtime=runtime, evidence_reader=evidence), ledger


async def test_quote_success_records_reserved_then_confirmed(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    evidence = _Evidence()
    ...
    assert terminal.details["quote_post_id"] == "999"
    assert terminal.details["quote_attachment_verified"] is True
...
async def test_empty_quote_text_can_confirm_with_explicit_attachment_evidence(
    tmp_path: Path,
) -> None:
    broker = _QuoteBroker()
    evidence = _Evidence(expected_text="")
    executor, ledger = _executor(tmp_path, broker=broker, evidence=evidence)

    execution = await executor.execute(_intent(text=""))
...
async def test_quote_target_id_only_uses_canonical_layer4_url(tmp_path: Path) -> None:
    broker = _QuoteBroker()
...
async def test_strong_quote_evidence_confirms_uncertain_click(tmp_path: Path) -> None:
    broker = _QuoteBroker(mode="fail_after_gate")
...
async def test_missing_explicit_quote_attachment_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
        evidence=_Evidence(attachment_verified=False),
    )
    ...
    assert execution.result.data["quote_attachment_verified"] is False
    terminal = ledger.read_records()[-1]
    assert terminal.state is EffectState.EFFECT_UNKNOWN
    assert terminal.details["quote_attachment_verified"] is False


async def test_quote_capture_failure_after_submit_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
...
async def test_quote_attachment_verification_failure_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
...
async def test_quote_ok_evidence_without_actor_still_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
...
async def test_quote_ok_evidence_without_verified_url_still_records_unknown(
    tmp_path: Path,
) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
...
async def test_quote_capture_cannot_reuse_target_id(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
        evidence=_Evidence(quote_post_id="123"),
    )
...
async def test_quote_capture_interruption_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
    ...
    assert execution.attempt_state is AttemptState.EFFECT_UNKNOWN
    assert execution.result.data["evidence_phase"] == "capture_quote_status"
    assert ledger.read_records()[-1].state is EffectState.EFFECT_UNKNOWN


async def test_quote_verification_interruption_records_unknown(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
    ...
    assert execution.attempt_state is AttemptState.EFFECT_UNKNOWN
    assert execution.result.data["evidence_phase"] == "verify_quote_attachment"
    assert ledger.read_records()[-1].state is EffectState.EFFECT_UNKNOWN


async def test_quote_baseline_interruption_is_clean_precommit(tmp_path: Path) -> None:
    broker = _QuoteBroker()
    executor, ledger = _executor(
        tmp_path,
        broker=broker,
...
async def test_quote_readback_interruption_is_clean_precommit(tmp_path: Path) -> None:
    broker = _QuoteBroker(mode="readback_raise")
    executor, ledger = _executor(tmp_path, broker=broker, evidence=_Evidence())
...
async def test_quote_target_open_failure_is_clean_precommit(tmp_path: Path) -> None:
    broker = _QuoteBroker(mode="open_failure")
    executor, ledger = _executor(tmp_path, broker=broker, evidence=_Evidence())
